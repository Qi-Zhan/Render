...
template <typename T> T polevl(const T x, const T A[], size_t len) {
  T result = 0;
...
int main() {
  float z = 0.10000000149011612;
  float result = calc_digamma(z);
  std::cout.precision(8);
  std::cout << "Result of float " << result << std::endl;
  double z2 = z;
  float result2 = (float)calc_digamma(z2);
  std::cout.precision(8);
  std::cout << "Result of double " << result2 << std::endl;
}