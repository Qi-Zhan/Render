...
class InterpreterBuilder:
    ...
    def fake_create_reduce(self, input, axis, combine_fn):
        ...

    def fake_create_scan(self, input, axis, combine_fn):
        ...
    ...
    def store_interval(self, *args, **kwargs):
        ...
...
class ReduceOps(ReduceScanOpIneterface):
    ...
    def apply_impl(self, input):
        if self.combine_fn == tl.standard._argmin_combine_tie_break_left:
            value = self.min_max(input[0], val_reduce_op=np.min, idx_reduce_op=np.argmin)
        elif self.combine_fn == tl.standard._argmax_combine_tie_break_left:
            value = self.min_max(input[0], val_reduce_op=np.max, idx_reduce_op=np.argmax)
        elif self.combine_fn == tl.standard._elementwise_max:
            value = self.min_max(input[0], val_reduce_op=np.max, idx_reduce_op=None)
        elif self.combine_fn == tl.standard._elementwise_min:
            value = self.min_max(input[0], val_reduce_op=np.min, idx_reduce_op=None)
        elif self.combine_fn == tl.standard._sum_combine:
            value = self.sum(input[0])
        else:
            # Fall back to the slow mode
            value = self.generic_reduce(input)
        interpreter_builder.fake_create_reduce(self, input, value)
        return value
...
class ScanOps(ReduceScanOpIneterface):
    ...
    def apply_impl(self, input):
        ...
        interpreter_builder.fake_create_scan(self, new_input, ret)
...
class InterpretedFunction:
    ...
    def __init__(self, fn, **kwargs) -> None:
        ...
        self.builder = interpreter_builder
...
from .interval import Interval, exp, exp2, log, log2, sqrt, sin, cos, cast_eps, IntervalArray
cos_interval = np.frompyfunc(lambda x: cos(x), 1, 1)
exp_interval = np.frompyfunc(lambda x: exp(x), 1, 1)
exp2_interval = np.frompyfunc(lambda x: exp2(x), 1, 1)
log_interval = np.frompyfunc(lambda x: log(x), 1, 1)
log2_interval = np.frompyfunc(lambda x: log2(x), 1, 1)
sqrt_interval = np.frompyfunc(lambda x: sqrt(x), 1, 1)
sin_interval = np.frompyfunc(lambda x: sin(x), 1, 1)

make_interval = np.frompyfunc(lambda x, eps: Interval.from_float(x, eps), 2, 1)
cast_eps_interval = np.frompyfunc(lambda x, eps: cast_eps(x, eps), 2, 1)
in_interval = np.frompyfunc(lambda x, y: x in y, 2, 1)
left_interval = np.frompyfunc(lambda x: x.left, 1, 1)
right_interval = np.frompyfunc(lambda x: x.right, 1, 1)
left_abs_interval = np.frompyfunc(lambda x: -abs(x.left), 1, 1)

def assert_in(a, b, *inputs):
    return
    index = 0
        ...
        for (i, j) in zip(a.flat, b.flat):
            if isinstance(j, Interval):
                if i not in j:
                    print(f"{i} not in {j}")
                    breakpoint()
                assert i in j, f"{i} not in {j}"
            index += 1
        
class IntervalInterpreterBuilder(InterpreterBuilder):
    def __init__(self) -> None:
        super().__init__()
        self.cache = {}
        self.store = []
    ...
    def create_masked_load(self, ptrs, mask, other, cache_modifier, eviction_policy, is_volatile):
        result = super().create_masked_load(ptrs, mask, other, cache_modifier, eviction_policy, is_volatile)
        eps = type2eps(result.data.dtype)
        data_sym = make_interval(result.data, eps)
        self.cache[result.get_id()] = data_sym
        return result
    
    def create_masked_store(self, ptrs, value, mask, cache_modifier, eviction_policy):
        return super().create_masked_store(ptrs, value, mask, cache_modifier, eviction_policy)
    
    def create_bitcast(self, src, dst_type):
        print('bitcast')
        breakpoint()
        result = super().create_bitcast(src, dst_type)
        return result
    
    def create_erf(self, arg):
        print('erf')
        breakpoint()
        result = super().create_erf(arg)
        return result
    
    def create_rsqrt(self, arg):
        print('rsqrt')
        breakpoint()
        return super().create_rsqrt(arg)
        
    def create_trans(self, arg, perm):
        result = super().create_trans(arg, perm)
        arg_sym = self.cache.get(arg.get_id())
        arg_sym = np.transpose(arg_sym, perm)
        self.cache[result.get_id()] = arg_sym
        return result
    
    def create_expand_dims(self, arg, axis):
        result = super().create_expand_dims(arg, axis)
        arg_sym = self.cache.get(arg.get_id(), arg.data)
        self.cache[result.get_id()] = np.expand_dims(arg_sym, axis)
        return result
    
    def create_broadcast(self, arg, shape):
        result = super().create_broadcast(arg, shape)
        arg_sym = self.cache.get(arg.get_id(), arg.data)
        self.cache[result.get_id()] = np.broadcast_to(arg_sym, shape)
        return result
    
    def create_cat(self, lhs, rhs):
        breakpoint()
        return super().create_cat(lhs, rhs)
    
    def create_join(self, lhs, rhs):
        breakpoint()
        return super().create_join(lhs, rhs)

    def create_split(self, val):
        breakpoint()
        return super().create_split(val)
    
    def create_splat(self, arg, shape):
        result = super().create_splat(arg, shape)
        self.cache[result.get_id()] = result.data
        return result

    def create_extern_elementwise(self, libName, libPath, symbol, argList, retType, isPure):
        breakpoint()
        return super().create_extern_elementwise(libName, libPath, symbol, argList, retType, isPure)
    
    def create_fma(self, a, b, c):
        print('fma')
        breakpoint()
        result = super().create_fma(a, b, c)
        return result

    def create_fp_to_fp(self, src, dst_type, rounding_mode):
        print('fp_to_fp')
        breakpoint()
        value = super().create_fp_to_fp(src, dst_type, rounding_mode)
        return value

    def create_dot(self, a, b, d, input_precision, max_num_imprecise_acc):
        value = super().create_dot(a, b, d, input_precision, max_num_imprecise_acc)
        aid, bid, did = a.get_id(), b.get_id(), d.get_id()
        a_sym = self.cache.get(aid, a.data)
        b_sym = self.cache.get(bid, b.data)
        d_sym = self.cache.get(did, d.data)
        # input_precision = input_precision.name
        if a.data.dtype == np.float16 and b.data.dtype == np.float16:
            eps = np.finfo(np.float32).eps
            a_sym = cast_eps_interval(a_sym, eps)
            b_sym = cast_eps_interval(b_sym, eps)
            d_sym = cast_eps_interval(d_sym, eps)
            value_sym = np.zeros((a_sym.shape[0], b_sym.shape[1]), dtype=object)
            for i in range(a_sym.shape[0]):
                for j in range(b_sym.shape[1]):
                    mul_result = np.multiply(a_sym[i, :], b_sym[:, j])
                    # left = left_abs_interval(mul_result).argsort()
                    # sorted_mul_result = mul_result[left]
                    sorted_mul_result = mul_result
                    value_sym[i, j] = np.sum(sorted_mul_result)
            if np.all(d != 0):
                value_sym = value_sym + d_sym
            assert_in(value.data, value_sym)
        elif a.data.dtype == np.float32 and b.data.dtype == np.float32: # TF32
            assert len(a_sym.shape) == 2 and len(b_sym.shape) == 2, f"Matrix multiplication not supported for shapes {a_sym.shape} and {b_sym.shape}"
            add_eps = np.finfo(np.float32).eps
            # triton chop the eps to half
            # cast to low precison
            mul_eps = np.finfo(np.float16).eps
            a_sym = cast_eps_interval(a_sym, mul_eps)
            b_sym = cast_eps_interval(b_sym, mul_eps)
            d_sym = cast_eps_interval(d_sym, mul_eps)
            value_sym = np.zeros((a_sym.shape[0], b_sym.shape[1]), dtype=object)
            for i in range(a_sym.shape[0]):
                for j in range(b_sym.shape[1]):
                    mul_result = np.multiply(a_sym[i, :], b_sym[:, j])
                    # cast to high precision
                    mul_result = cast_eps_interval(mul_result, add_eps)
                    sorted_mul_result = mul_result
                    # left = left_abs_interval(mul_result).argsort()
                    # sorted_mul_result = mul_result[left]
                    value_sym[i, j] = np.sum(sorted_mul_result)
            if np.all(d != 0):
                d_sym = cast_eps_interval(d_sym, add_eps)
                value_sym += d_sym
            assert_in(value.data, value_sym)
        else:
            raise NotImplementedError(f"Matrix multiplication not implemented for {a.data.dtype} and {b.data.dtype}")
        self.cache[value.get_id()] = value_sym
        assert_in(value.data, value_sym)
        return value

    def binary_op(self, lhs, rhs, op):
        value = super().binary_op(lhs, rhs, op)
        lid, rid = lhs.get_id(), rhs.get_id()
        lhs_sym = self.cache.get(lid, lhs.data) # TODO: data?
        rhs_sym = self.cache.get(rid, rhs.data)
        if op == np.maximum or op == np.minimum:
            lhs_data, rhs_data = lhs.data, rhs.data
            select_from = np.where(value.data == lhs_data, 'lhs', 'rhs')
            result = np.where(select_from == 'lhs', lhs_sym, rhs_sym)
            # result = np.where(select_from == 'lhs', lhs_data, rhs_data)
            self.cache[value.get_id()] = result
            assert_in(value.data, self.cache[value.get_id()])
        else:
            self.cache[value.get_id()] = op(lhs_sym, rhs_sym)
            assert_in(value.data, self.cache[value.get_id()], lhs_sym, rhs_sym, lhs.data, rhs.data)
        return value

    def unary_op(self, arg, op):
        value = super().unary_op(arg, op)
        aid = arg.get_id()
        arg_sym = self.cache.get(aid, arg.data) # TODO: data?
        match op:
            case np.cos: value_sym = cos_interval(arg_sym)
            case np.exp: value_sym = exp_interval(arg_sym)
            case np.exp2:
                if isinstance(arg_sym.flat[0], np.float32) and np.isinf(arg_sym).all():
                    if arg_sym.flat[0] == np.inf:
                        raise ValueError("exp2(inf) is infinity")
                    value_sym = np.full_like(arg_sym, 0.0)
                else:
                    value_sym = exp2_interval(arg_sym)
            case np.log: value_sym = log_interval(arg_sym)
            case np.log2: value_sym = log2_interval(arg_sym)
            case np.sqrt: value_sym = sqrt_interval(arg_sym)
            case np.sin: value_sym = sin_interval(arg_sym)
            case _: raise NotImplementedError(f"Unary operation {op} not implemented")
        self.cache[value.get_id()] = value_sym
        assert_in(value.data, self.cache[value.get_id()])
        return value
    
    def cast_impl(self, src, dst_type):
        value = super().cast_impl(src, dst_type)
        value_sym = self.cache.get(src.get_id(), value.data)
        if isinstance(dst_type, tl.block_type):
            dst_type = dst_type.element_ty
        if dst_type.is_floating():
            match dst_type:
                case tl.float16: eps = np.finfo(np.float16).eps
                case tl.float32: eps = np.finfo(np.float32).eps
                case tl.float64: eps = np.finfo(np.float64).eps
                case _: raise NotImplementedError(f"Cast to {dst_type} not implemented")
            value_sym = cast_eps_interval(value_sym, eps)
        self.cache[value.get_id()] = value_sym
        return value 
    ...
    def create_fp_to_fp(self, src, dst_type, rounding_mode):
        value = super().create_fp_to_fp(src, dst_type, rounding_mode)
        value_sym = self.cache.get(src.get_id(), value.data)
        eps = type2eps(dst_type)
        value_sym = cast_eps_interval(value_sym, eps)
        self.cache[value.get_id()] = value_sym
        return value

    def fake_create_scan(self, reduce, input, result):
        combine_fn = reduce.combine_fn
        cache = self.cache
        input = input[0].handle
        result = result[0].handle
        arg_error = cache[input.get_id()]
        if combine_fn == tl.standard._prod_combine: # cumprod
            error = np.cumprod(arg_error, axis=reduce.axis)
        elif combine_fn == tl.standard._sum_combine: # cumsum
            error = np.cumsum(arg_error, axis=reduce.axis)
        else:
            raise NotImplementedError(f"Combine function {combine_fn} not implemented")
        assert error.shape == result.data.shape, f"Error shape {error.shape} does not match data shape {result.data.shape}"
        cache[result.get_id()] = error
    
    def create_select(self, cond, lhs, rhs):
        value = super().create_select(cond, lhs, rhs)
        lhs_sym = self.cache.get(lhs.get_id())
        rhs_sym = self.cache.get(rhs.get_id())
        self.cache[value.get_id()] = np.where(cond.data, lhs_sym, rhs_sym)
        return value
    
    def fake_create_reduce(self, reduce, input, result):
        combine_fn = reduce.combine_fn
        cache = self.cache
        input = input[0].handle
        result = result.handle
        arg_error = cache[input.get_id()]
        assert arg_error.shape == input.data.shape, f"Error shape {arg_error.shape} does not match data shape {input.data.shape}"
        if combine_fn == tl.standard._argmin_combine_tie_break_left:
            return
        elif combine_fn == tl.standard._argmax_combine_tie_break_left:
            return
        elif combine_fn == tl.standard._elementwise_max:
            index = np.argmax(input.data, axis=reduce.axis, keepdims=True)
            error = np.take_along_axis(arg_error, index, axis=reduce.axis)
            error = error.squeeze(axis=reduce.axis)
        elif combine_fn == tl.standard._elementwise_min:
            index = np.argmin(input.data, axis=reduce.axis, keepdims=True)
            error = np.take_along_axis(arg_error, index, axis=reduce.axis)
            error = error.squeeze(axis=reduce.axis)
        elif combine_fn == tl.standard._sum_combine:
            error = np.sum(arg_error, axis=reduce.axis, keepdims=reduce.keep_dims)
        else:
            raise NotImplementedError(f"Combine function {combine_fn} not implemented")
        if isinstance(error, Interval) or len(error.shape) == 0:
            error = np.array([error])
        assert error.shape == result.data.shape, f"Error shape {error.shape} does not match data shape {result.data.shape}"
        cache[result.get_id()] = error
    
    def record_input(self, name: str, address):
        pass
    
    def record(self, address, value: TensorHandle, boundary_check=(), mask=None):
        data = value.data
        interval = self.cache[value.get_id()]
        if isinstance(address, BlockPointerHandle):
            address = address.materialize_pointers(boundary_check)[0]
        address = address.data
        if mask is not None:
            mask = mask.data
            if np.size(mask) > np.sum(mask):
                # only choose the masked elements
                new_interval = []
                new_address = []
                new_data = []
                for (a, d, i, m) in zip(address.flat, data.flat, interval.flat, mask.flat):
                    if m:
                        new_address.append(a)
                        new_data.append(d)
                        new_interval.append(i)
                data = np.array(new_data)
                address = np.array(new_address)
                interval = np.array(new_interval)
        self.store.append((address, data, interval))
    
    def show(self, f = sys.stdout):
        max_relative_error = 0
        for (_, data, interval) in self.store:
            for (d, i) in zip(data.flat, interval.flat):
                assert d in i, f"{d} not in {i}"
                if d == 0 and i.left == 0 and i.right == 0:
                    continue
                error1 = (d - i.left) / d
                error2 = (i.right - d) / d
                max_relative_error = max(max_relative_error, error1, error2)
            # error1 = (data - left_interval(interval)) / data
            # error2 = (right_interval(interval) - data) / data
            max_relative_error = max(max_relative_error, np.max(error1), np.max(error2))
        f.write(f"interval error {max_relative_error}\n")
    
    def store_interval(self):
        left = np.concatenate([left_interval(interval).flatten() for (_, _, interval) in self.store])
        right = np.concatenate([right_interval(interval).flatten() for (_, _, interval) in self.store])
        addresses = np.concatenate([address.flatten() for (address, _, _) in self.store])
        order = np.argsort(addresses, kind='stable') # keep the order of equal elements
        left = left[order]
        right = right[order]
...
# interpreter_builder = IntervalInterpreterBuilder()

class FastIntervalInterpreterBuilder(InterpreterBuilder):
    def __init__(self) -> None:
        super().__init__()
        self.cache: dict[int, IntervalArray] = {}
        self.store_address = []
        self.store_data = []
        self.store_intervals = []
    
    def clear(self):
        self.cache.clear()
        self.store_address.clear()
        self.store_data.clear()
        self.store_intervals.clear()
    
    def create_masked_load(self, ptrs, mask, other, cache_modifier, eviction_policy, is_volatile):
        result = super().create_masked_load(ptrs, mask, other, cache_modifier, eviction_policy, is_volatile)
        eps = type2eps(result.data.dtype)
        data_sym = IntervalArray.from_float(result.data, eps)
        self.cache[result.get_id()] = data_sym
        return result
    ...
    def create_trans(self, arg, perm):
        result = super().create_trans(arg, perm)
        arg_sym = self.cache.get(arg.get_id())
        arg_sym = arg_sym.transpose(perm)
        self.cache[result.get_id()] = arg_sym
        return result
    
    def create_expand_dims(self, arg, axis):
        result = super().create_expand_dims(arg, axis)
        arg_sym = self.cache.get(arg.get_id(), IntervalArray.from_constant(arg.data))
        if isinstance(arg_sym, np.bool_): # 1808
            return result
        if isinstance(arg_sym, IntervalArray):
            arg_sym = arg_sym.expand_dims(axis)
        else:
            arg_sym = IntervalArray.from_constant(np.expand_dims(arg_sym, axis))
        self.cache[result.get_id()] = arg_sym
        return result
    ...
    def create_broadcast(self, arg, shape):
        ...
        arg_sym = self.cache.get(arg.get_id(), IntervalArray.from_constant(arg.data))
        if isinstance(arg_sym, IntervalArray):
            arg_sym = arg_sym.broadcast_to(shape)
        else:
            arg_sym = np.broadcast_to(arg_sym, shape)
        self.cache[result.get_id()] = arg_sym
        return result
    ...
    def create_dot(self, a, b, d, input_precision, max_num_imprecise_acc):
        value = super().create_dot(a, b, d, input_precision, max_num_imprecise_acc)
        aid, bid, did = a.get_id(), b.get_id(), d.get_id()
        a_sym = self.cache.get(aid, IntervalArray.from_constant(a.data))
        b_sym = self.cache.get(bid, IntervalArray.from_constant(b.data))
        d_sym = self.cache.get(did, IntervalArray.from_constant(d.data))
        if a.data.dtype == np.float16 and b.data.dtype == np.float16:
            mul_eps = np.finfo(np.float32).eps
            add_eps = np.finfo(np.float32).eps
        elif a.data.dtype == np.float32 and b.data.dtype == np.float32: # TF32
            mul_eps = np.finfo(np.float16).eps
            add_eps = np.finfo(np.float32).eps
        # Unused, as the data interpretation is wrong
        # elif a.data.dtype == np.uint8 and b.data.dtype == np.uint8: # fp8 * fp8
        #     mul_eps = np.finfo(np.float32).eps
        #     add_eps = np.finfo(np.float32).eps
        else:
            raise NotImplementedError(f"Matrix multiplication not implemented for {a.data.dtype} and {b.data.dtype}")
        mul_eps, add_eps = float(mul_eps), float(add_eps)
        # value_sym = a_sym.matmul_without_mixed_precision(b_sym, d_sym, mul_eps, add_eps, d)
        value_sym = a_sym.matmul(b_sym, d_sym, mul_eps, add_eps, d)
        self.cache[value.get_id()] = value_sym
        return value
    ...
    def binary_op(self, lhs, rhs, op):
        value = super().binary_op(lhs, rhs, op)
        lid, rid = lhs.get_id(), rhs.get_id()
        lhs_sym = self.cache.get(lid, IntervalArray.from_constant(lhs.data)) 
        rhs_sym = self.cache.get(rid, IntervalArray.from_constant(rhs.data))
        if op == np.maximum or op == np.minimum:
            lhs_data, _ = lhs.data, rhs.data
            result = lhs_sym.select(rhs_sym, value.data == lhs_data)
            self.cache[value.get_id()] = result
            # assert value.data in self.cache[value.get_id()]
        else:
            result = op(lhs_sym, rhs_sym)
            self.cache[value.get_id()] = result
            # if isinstance(result, IntervalArray):
            #     assert value.data in self.cache[value.get_id()], f"{value.data} not in {self.cache[value.get_id()]}"
        return value

    def unary_op(self, arg, op):
        value = super().unary_op(arg, op)
        aid = arg.get_id()
        arg_sym = self.cache.get(aid, IntervalArray.from_constant(arg.data))
        match op:
            case np.cos: value_sym = arg_sym.cos()
            case np.exp: value_sym = arg_sym.exp()
            case np.exp2: value_sym = arg_sym.exp2()
            case np.log: value_sym = arg_sym.log()
            case np.log2: value_sym = arg_sym.log2()
            case np.sqrt: value_sym = arg_sym.sqrt()
            case np.sin: value_sym = arg_sym.sin()
            case _: raise NotImplementedError(f"Unary operation {op} not implemented")
        self.cache[value.get_id()] = value_sym
        # assert value.data in value_sym
        return value
    
    def cast_impl(self, src, dst_type):
        value = super().cast_impl(src, dst_type)
        value_sym = self.cache.get(src.get_id(), IntervalArray.from_constant(value.data))
        if isinstance(dst_type, tl.block_type):
            dst_type = dst_type.element_ty
        if dst_type.is_floating():
            match dst_type:
                ...
                case tl.float64: eps = np.finfo(np.float64).eps
                case _: raise NotImplementedError(f"Cast to {dst_type} not implemented")
            value_sym = value_sym.cast_eps(eps)
        self.cache[value.get_id()] = value_sym
        return value 
    ...
    def fake_create_scan(self, reduce, input, result):
        ...
        arg_error = cache[input.get_id()]
        if combine_fn == tl.standard._prod_combine: # cumprod
            error = arg_error.cumprod(axis=reduce.axis)
        elif combine_fn == tl.standard._sum_combine: # cumsum
            error = arg_error.cumsum(axis=reduce.axis)
        else:
            raise NotImplementedError(f"Combine function {combine_fn} not implemented")
        # assert error.shape == result.data.shape, f"Error shape {error.shape} does not match data shape {result.data.shape}"
        cache[result.get_id()] = error
    ...
    def create_select(self, cond, lhs, rhs):
        value = super().create_select(cond, lhs, rhs)
        lhs_sym = self.cache.get(lhs.get_id(), IntervalArray.from_constant(lhs.data))
        rhs_sym = self.cache.get(rhs.get_id(), IntervalArray.from_constant(rhs.data))
        self.cache[value.get_id()] = lhs_sym.select(rhs_sym, cond.data)
        return value
    ...
    def fake_create_reduce(self, reduce, input, result):
        ...
        if combine_fn == tl.standard._argmin_combine_tie_break_left:
            ...
            index = np.argmax(input.data, axis=reduce.axis, keepdims=True)
            error = arg_error.take_along_axis(index, axis=reduce.axis)
            error = error.squeeze(axis=reduce.axis)
        elif combine_fn == tl.standard._elementwise_min:
            index = np.argmin(input.data, axis=reduce.axis, keepdims=True)
            error = arg_error.take_along_axis(index, axis=reduce.axis)
            ...
            error = arg_error.sum(axis=reduce.axis, keepdims=reduce.keep_dims)
        else:
            raise NotImplementedError(f"Combine function {combine_fn} not implemented")
        # assert error.shape == result.data.shape, f"Error shape {error.shape} does not match data shape {result.data.shape}"
        cache[result.get_id()] = error
    ...
    def record(self, address, value: TensorHandle, boundary_check=(), mask=None):
        ...
        if mask is not None:
            ...
            data = data[mask]
            interval = interval.index(mask)
        self.store_address.append(address)
        self.store_data.append(data)
        self.store_intervals.append(interval)
    
    def show(self, f = sys.stdout):
        max_relative_error = 0
        for (data, interval) in zip(self.store_data, self.store_intervals):
    ...
    def store_interval(self):
        left = np.concatenate([interval.lo.flatten() for interval in self.store_intervals])
        right = np.concatenate([interval.hi.flatten() for interval in self.store_intervals])
        addresses = np.concatenate([address.flatten() for address in self.store_address])
        order = np.argsort(addresses, kind='stable') # keep the order of equal elements
        left = left[order]
        right = right[order]
        # with open("interval_RQ3.npy", "wb") as f:
...
interpreter_builder = FastIntervalInterpreterBuilder()